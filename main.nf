...
process compute_simple_ratios {
  ...
  for f in ${sample_count_files}; do
    ...
    # Final EHP call requires concordant mapping evidence from both independent
    # targets. Any INC target prevents a confirmed EHP call.
    if [[ "\$ehp_ssu_status" == "INC" || "\$ehp_swp_status" == "INC" ]]; then
      ...
      ehp_interpretation="EHP_CONFIRMED_BY_MAPPING"
      overall="POS"
      num_pos=\$((num_pos+1))
      pos_list=\${pos_list:+\$pos_list,}EHP
    elif [[ "\$ehp_ssu_status" == "POS" && "\$ehp_swp_status" == "NEG" ]]; then
      ehp_interpretation="EHP_SUSPECT_SSU_ONLY"
      [[ "\$overall" != "POS" ]] && overall="INC"
      num_inc=\$((num_inc+1))
      inc_list=\${inc_list:+\$inc_list,}EHP
    elif [[ "\$ehp_ssu_status" == "NEG" && "\$ehp_swp_status" == "POS" ]]; then
...
<style>
  ...
  .EHP_CONFIRMED_BY_MAPPING{ background:var(--pos-bg); color:var(--pos-fg); }
